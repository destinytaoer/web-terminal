...
export async function parseFileSystemHandle(pathId: number, handle: FileSystemHandle, fileList: BrowserFileReader[], relPath: string[]) {
  if (handle.kind === 'file') {
...
export async function openSaveFile(saveParam: any, fileName: string, directory: boolean, overwrite: boolean) {
  ...
  if (!file.hasOwnProperty('path_name') || !file.hasOwnProperty('path_id') || !file.hasOwnProperty('is_dir') || file.path_name.length < 1) {
