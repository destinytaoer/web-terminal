...
const MIN_ZM_HEX_START_LENGTH = 20,
  ...
  SENTRY_CONSTRUCTOR_REQUIRED_ARGS: (keyof SentryOptions)[] = ['to_terminal', 'on_detect', 'on_retract', 'sender'],
  ASTERISK = 42

type SessionType = 'receive' | 'send'
...
export class Detection {
   ...
   * Not called directly.
   */
  constructor(session_type: SessionType, accepter, denier, checker) {
...
interface SentryOptions {
  to_terminal: (octets: Octets) => void
  on_detect: (dection: Detection) => void
  on_retract: () => void
  sender: (octets: Octets) => void
}
...
export class ZmodemSentry {
   ...
   */
  consume(input: number[] | ArrayBuffer) {
  ...
  _parse(array_like) {
    ...
    while (true) {
      let common_hex_at = ZMLIB.find_subarray(cache, COMMON_ZM_HEX_START)
