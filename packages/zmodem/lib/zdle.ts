...
let encode_cur, encode_todo

const ZDLE = ZMLIB.ZDLE
 ...
 */
export class ZmodemZDLE {
  _config = {
    escape_ctrl_chars: false,
    turbo_escape: false,
  }
   ...
   */
  set_escape_ctrl_chars(value: boolean) {
    if (typeof value !== 'boolean') throw new Error('need boolean!')
  ...
  */
   ...
   */
  encode(octets: Octets) {
    //NB: Performance matters here!

    if (!this._zdle_table) throw new Error('No ZDLE encode table configured!')
    ...
    for (encode_cur = 0; encode_cur < octets.length; encode_cur++) {
      ...
      if (!encode_todo) {
        ...
        throw new Error('Invalid octet: ' + octets[encode_cur])
      }
    ...
    octets.splice(0)
    octets.push(...new Uint8Array(arrbuf, 0, arrbuf_i))
   ...
   */
  static splice(octets: Octets, offset: number, count: number) {
    let so_far = 0

    if (!offset) offset = 0

    let i = offset
    for (i = offset; i < octets.length && so_far < count; i++) {
      so_far++
  ...
  _setup_zdle_table() {
    const zsendline_tab = new Array(256)
