 ...
 */
class _Eventer {
  _on_evt: Record<string, EventCallback[]>

  _evt_once_index: Record<string, EventCallback[]>
  ...
  _Add_event(evt_name: string) {
    this._on_evt[evt_name] = []
    this._evt_once_index[evt_name] = []
  }

  _get_evt_queue(evt_name: string) {
    if (!this._on_evt[evt_name]) {
      throw new Error('Bad event: ' + evt_name)
    }
   ...
   */
  on(evt_name: string, todo: EventCallback) {
   ...
   *  is unregistered.
   */
  off(evt_name: string, todo: EventCallback) {
    const queue = this._get_evt_queue(evt_name)

    if (todo) {
      const at = queue.indexOf(todo)
      if (at === -1) {
        throw new Error('“' + todo + '” is not in the “' + evt_name + '” queue.')
      }
      queue.splice(at, 1)
    } else {
      queue.pop()
  ...
  _Happen(evt_name: string, ...args: any[]) {
...
type SendFunction = (data: string | Uint8Array) => void
 ...
 * @extends _Eventer
 */
export class ZmodemSession extends _Eventer {
  /**
  ...
  _trim_leading_garbage_until_header() {
    var garbage = ZmodemHeader.trim_leading_garbage(this._input_buffer)
  ...
  _parse_and_consume_header() {
    ...
    var new_header_and_crc = ZmodemHeader.parse(this._input_buffer)
  ...
  _consume_header(new_header) {
    ...
    if (!handler) {
      ...
      throw new ZmodemError('Unhandled header: ' + new_header.NAME)
    }
  ...
  _check_for_abort_sequence() {
    var abort_at = ZMLIB.find_subarray(this._input_buffer, ABORT_SEQUENCE)
  ...
  _create_header_bytes(name_and_args) {
    var hdr = ZmodemHeader.build.apply(ZmodemHeader, name_and_args)
  ...
  _strip_and_enqueue_input(input) {
    ZMLIB.strip_ignored_bytes(input)
...
function _trim_OO(array) {
  if (0 === ZMLIB.find_subarray(array, OVER_AND_OUT)) {
 ...
 * @extends Session
 */
export class ZmodemReceiveSession extends ZmodemSession {
  ...
  _parse_and_consume_subpacket() {
    ...
    var subpacket = ZmodemSubpacket[parse_func](this._input_buffer)
  ...
  _consume_first() {
    if (this._got_ZFIN) {
      ...
      if (ZMLIB.find_subarray(this._input_buffer, OVER_AND_OUT) !== 0) {
  ...
  _octets_to_string(octets) {
    if (!this._textdecoder) {
      this._textdecoder = new Text.Decoder()
    }
  ...
  _consume_ZFILE_data(hdr, subpacket) {
    ...
    var xfer = new Offer(hdr.get_options(), this._file_info, this._accept.bind(this), this._skip.bind(this))
...
Object.assign(ZmodemReceiveSession.prototype, {
  type: 'receive',
})
 ...
 */
export class Transfer extends _Eventer {
  /**
   * Not called directly.
   */
 ...
 * @mixes Transfer_Offer_Mixin
 */
export class Offer extends _Eventer {
  /**
   * Not called directly.
   */
 ...
 * @extends Session
 */
export class ZmodemSendSession extends ZmodemSession {
  /**
   * Not called directly.
   */
  ...
  _convert_params_to_offer_payload_array(params) {
    params = Validation.offer_parameters(params)
      ...
      var handler_setter_promise = new Promise(function (res) {
        sess._next_header_handler = {
          ...
          ZRPOS: function (hdr) {
            ...
            let transfer = new Transfer(params, hdr.get_offset(), sess._send_interim_file_piece.bind(sess), sess._end_file.bind(sess))
  ...
  _build_subpacket_bytes(bytes_arr, frameend) {
    var subpacket = ZmodemSubpacket.build(bytes_arr, frameend)
  ...
  _string_to_octets(string) {
    if (!this._textencoder) {
      this._textencoder = new Text.Encoder()
    }
...
Object.assign(ZmodemSendSession.prototype, {
  type: 'send',
})
