...
const HEX_HEADER_CRLF_XON = HEX_HEADER_CRLF.slice(0).concat([ZMLIB.XON])

//These are more or less duplicated by the logic in trim_leading_garbage().
//
//"**" + ZDLE_CHAR + "B"
const HEX_HEADER_PREFIX = [ZPAD, ZPAD, ZMLIB.ZDLE, ZHEX]
const BINARY16_HEADER_PREFIX = [ZPAD, ZMLIB.ZDLE, ZBIN]
const BINARY32_HEADER_PREFIX = [ZPAD, ZMLIB.ZDLE, ZBIN32]

/** Class that represents a ZMODEM header. */
export class ZmodemHeader {
  _hex_header_ending = HEX_HEADER_CRLF_XON

  static parse_hex = _parse_hex
   ...
   */
  static trim_leading_garbage(ibuffer: Octets) {
    ...
    let discard_all, parser
    const next_ZPAD_at_least = 0

    TRIM_LOOP: while (ibuffer.length && !parser) {
      ...
      if (first_ZPAD === -1) {
        ...
        garbage.push(...ibuffer.splice(0, first_ZPAD))
   ...
   */
  static build(name: string /*, args */) {
    const args = arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments)

    //TODO: make this better
    const Ctr = FRAME_NAME_CREATOR[name]
    if (!Ctr) throw new Error('No frame class “' + name + '” is defined!')
  ...
  to_hex() {
    ...
    return HEX_HEADER_PREFIX.concat(ENCODELIB.octets_to_hex(to_crc.concat(CRC.crc16(to_crc))), this._hex_header_ending)
  }
  ...
  _to_binary(zencoder, prefix, crc_func) {
    ...
    var octets = prefix.concat(zencoder.encode(to_crc.concat(crc_func(to_crc))))
...
class ZRQINIT_HEADER extends ZmodemHeader {}
...
class ZRINIT_HEADER extends ZmodemHeader {
  ...
  get_buffer_size() {
    return ENCODELIB.unpack_u16_be(this._bytes4.slice(0, 2)) || undefined
  }
...
class ZSINIT_HEADER extends ZmodemHeader {
...
class ZACK_HEADER extends ZmodemHeader {
...
class ZFILE_HEADER extends ZmodemHeader {
...
class ZSKIP_HEADER extends ZmodemHeader {}

//No need for ZNAK
class ZABORT_HEADER extends ZmodemHeader {}

class ZFIN_HEADER extends ZmodemHeader {}

class ZFERR_HEADER extends ZmodemHeader {}

ZFIN_HEADER.prototype._hex_header_ending = HEX_HEADER_CRLF

class ZOffsetHeader extends ZmodemHeader {
  constructor(offset) {
    super()
    this._bytes4 = ENCODELIB.pack_u32_le(offset)
  }

  get_offset() {
    return ENCODELIB.unpack_u32_le(this._bytes4)
  }
}
...
//class ZCAN_HEADER extends ZmodemHeader {}
...
function _get_blank_header(typenum: number) {
...
function _parse_binary16(bytes_arr: Octets) {
  ...
  const zdle_decoded = ZmodemZDLE.splice(bytes_arr, BINARY16_HEADER_PREFIX.length, 7)

  return zdle_decoded && _parse_non_zdle_binary16(zdle_decoded)
}

function _parse_non_zdle_binary16(decoded: Octets) {
  CRC.verify16(decoded.slice(0, 5), decoded.slice(5))
...
function _parse_binary32(bytes_arr: Octets) {
  ...
  CRC.verify32(zdle_decoded.slice(0, 5), zdle_decoded.slice(5))
...
function _parse_hex(bytes_arr: Octets) {
  ...
  //  4 for header data
  //  2 for CRC
  const octets = ENCODELIB.parse_hex_octets(hex_bytes)
