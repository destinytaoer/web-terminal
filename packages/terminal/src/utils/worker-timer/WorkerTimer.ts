...
export class WorkerTimer {
  ...
  constructor() {
    this._worker.addEventListener('message', (ev: MessageEvent<CallMessage>) => {
      this._callId(ev)
    })
  }
  ...
  private _callId(ev: MessageEvent<CallMessage>) {
    ...
    typeof record.cb === 'string' ? eval(record.cb)(...record.args) : record.cb(...record.args)

    if (record.method === 'timeout') this._cbMap.delete(id)
  }

  public setWorkerTimer(method: TimerMethod, cb: TimerCallback, delay?: number, ...args: any[]): number {
